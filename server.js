...
app.get('/player_info/select_player',player_id, function(req,res) {
    var query1 = 'SELECT name,id FROM football_players;';
    var query2 = 'SELECT name FROM football_players WHERE id = player_id ';
    var query3 = 'SELECT COUNT(players) FROM football_games WHERE (SELECT id FROM football_players WHERE id= (player_id) = ANY(players);';
    db.task('get-everything', task => {
        return task.batch([
            task.any(query1),
            task.any(query2),
